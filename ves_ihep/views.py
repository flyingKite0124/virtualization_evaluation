...
def add_host(request):
	if request.is_ajax()==False and request.method == 'POST':
		...
		host.status=0
		host.save()
		if ves_connection.test.ssh_cmd(host.IP,22,host.username,host.passwd,"echo success"):
			if ves_connection.test.check_file(host.IP,22,host.username,host.passwd):
				pass
			else:
				ves_connection.test.ssh_cmd(host.IP,22,host.username,host.passwd,"mkdir /ves_ihep_scripts")
				ves_connection.test.scp_cmd(host.IP,22,host.username,host.passwd,os.path.join(os.path.dirname(os.path.abspath(__file__)),"ves_connection","server.py"))
				ves_connection.test.ssh_cmd(host.IP,22,host.username,host.passwd,"chmod 755 /ves_ihep_scripts/server.py")
				ves_connection.test.ssh_cmd(host.IP,22,host.username,host.passwd,"/ves_ihep_scripts/server.py")
    ...
    if request.is_ajax()==True and request.method == "POST":
        ...
        for host in hosts:
            if host.status==2:
                ...
                ret['hosts'][host.id]=ves_connection.test.ssh_cmd(host.IP,22,host.username,host.passwd,"echo success")
...
def add_activity(request):
    if request.method == "POST":
        ...
        if request.POST['script_type'] == "pool":
            ...
            script = Script()
            script.upload_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%I:%S")
            script.script_name = str(script.upload_time) + request.FILES['Native_Act_File'].name
            script.script_type = "native"
            script.script_path = os.path.join(os.path.dirname(os.path.dirname(__file__)),"scripts",request.FILES['Native_Act_File'].name)
            script.save()
            fp = open(script.script_path,"w")
...
def get_activity(request):
    if request.is_ajax()==True and request.method == "POST":
        ...
        return JsonResponse({'result':'success','activities':ret_activities})
...
def deploy(request):
    if request.is_ajax()==True and request.method == "POST":
        ...
        for host_id in hosts:
        	...
        	thread.start_new_thread(run_activity,(scene_history,host,activities))
        return JsonResponse({'result':'success'})
...
def view_result(request):
    if request.is_ajax()==True and request.method == "POST":
        activity_history=ActivityHistory.objects.get(pk=request.POST['activity_history_id'])
        return JsonResponse({'result':'success','content':activity_history.result})
...
def evaresult(request):
    if request.is_ajax()==False and request.method == 'GET':
        ...
        for _scene_history in temp_var['scene_histories']:
            _scene_history.activity_histories=ActivityHistory.objects.filter(scene_history=_scene_history)
        context=RequestContext(request,temp_var)
        return HttpResponse(template.render(context))
    else:
