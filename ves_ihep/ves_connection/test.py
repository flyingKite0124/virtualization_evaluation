...
def scp_cmd(ip, port, user, passwd, filename):
    scp = pexpect.spawn(
        'scp -P%s %s %s@%s:/ves_ihep_scripts/' %
        (port, filename, user, ip))
    try:
        index = scp.expect(
...
def ssh_cmd(ip, port, user, passwd, cmd):
    ...
    try:
        index = ssh.expect(
            ['password:', 'continue connecting (yes/no)?'], timeout=2)
        if index == 0:
            ssh.sendline(passwd)
        elif index == 1:
            ssh.sendline("yes")
...
def check_file(ip, port, user, passwd):
    ssh = pexpect.spawn('ssh %s@%s -p%s "%s"' %
                        (user, ip, port, "ls / | grep ves_ihep_scripts"))
    r = ''
    try:
        index = ssh.expect(
            ['password:', 'continue connecting (yes/no)?'], timeout=2)
...
if __name__ == "__main__":
    ...
    passwd = '#srv@309'
    # test add_host
    # ps:ssh_cmd(ip,port,user,passwd,"/ves_ihep_scripts/server.py") must
    # TIMEOUT
    ...
    scp_cmd(
        ...
        user,
        passwd,
        "/home/lv/virtualization_evaluation/ves_ihep/ves_connection/server.py")
